...
Loops House is the permanent home for developer projects. Every project built during a Booster, Residency — or independently — deserves a rich, discoverable profile with AI-generated insights, live metrics, and an AI representative powered by its own knowledge base.
...
* Booster Hosts: End-to-end developer event management with problem statement curation (RFS), AI judging, builder analytics, and a custom Booster page.
...
| Booster Host | OAuth \+ Admin Approval | Create Boosters (all 3 types), view builder graph, configure judging, access analytics | Elevated Authenticated |
...
* As a builder, I must have a team before submitting a project to any Booster.
...
* As a builder, I can apply to a Booster individually or with my team.

* As a builder, I can submit a project to a Booster using an existing Loops Profile or by creating a new one during the application flow.

* As a builder, I can use the Project Ideator Agent while exploring a Booster to ideate project ideas based on the problem statement.

* As a builder, I can see all my Booster applications and their statuses in my dashboard.

* As a builder, I can see my AI Judge score and human judge remarks after the judging window closes.

## **3.3 Booster Hosts**

* As a prospective host, I can submit a Booster application via a public form (Booster type, name, dates, description, expected participants, prizes, problem statement/RFS, sponsor tracks, contact info).

* As an approved host, I receive a Booster workspace with a custom Booster Page auto-populated from my form.

* As a host, I can edit my Booster Page at any time: banner, timeline, rules, prizes, sponsor logos, judges, and FAQs.

* As a host, I can set custom judging criteria and weights for the AI Judge for my specific Booster type.
...
* As a host, I can enable or disable the live leaderboard for my Booster.

## **3.4 Judges**

* As a judge, I receive an invite link from the host to access the Booster's judging panel.

* As a judge, I can browse all submitted projects for my assigned Booster.
...
* As a viewer, I can filter projects by category~~, tech stack, Booster participated in, and date~~.
...
# **4\. Booster Types — Detailed Specifications**
...
Loops House uses a layered AI agent architecture. Sub-agents are internal utilities. Builder, Host, Viewer, and DevRel agents are user-facing. All agents are stateless; state is persisted in the knowledge base database keyed by project\_id or booster\_id.
...
| Project Ideator | Builder \+ DevRel Agent | Builder in Booster ideation flow | gemini-2.0-flash |
| Social Amplifier | Builder Agent | Builder on Loops Profile dashboard | gemini-2.0-flash |
| Tech Buddy | DevRel Agent | Builder in Booster support chat | gemini-1.5-pro |
...
* Flattens the codebase into a single concatenated document (excluding node\_modules, lock files, binaries).
...
* Exposes a queryCode(project\_id, question) interface reused by Code Query agent.
...
* If Booster context is provided, evaluates how well the demo addresses the problem statement.
...
* Outputs: primary\_color (hex), accent\_color (hex), theme\_label (e.g. 'dark-minimal').
...
* Saves unified knowledge base to pgvector store keyed by project\_id.

* Emits real-time SSE progress events to frontend per sub-agent completion.

Acceptance Criteria: Completes within 90 seconds. All AI fields are flagged ai\_generated=true and fully editable by builder. If any sub-agent fails, continue with available data.

## **5.3 Project Ideator Agent**

Conversational agent helping builders ideate, refine, and validate a project idea against a Booster's problem statement. Merged from project-ideator \+ project-mentor. Acts as a thinking partner, not a code assistant.

* Input: Builder message, conversation history, Booster context (problem statements, RFS, sponsor tracks).

* Asks clarifying questions about builder's skills, interests, and what problem they want to solve.

* Ties every suggestion back to one or more problem statements in the Booster context.

* Helps think through: uniqueness, feasibility within Booster timeline, judge appeal, sponsor track alignment.
...
* Input: Project name, tagline, description, tech stack, Booster name, result (winner/participant/etc).
...
Strict resource-grounded Q\&A assistant. Answers technical questions ONLY from sponsor documentation and technical cheatsheets uploaded by the Booster host. Not a programmer, not a debugger.

* Resource bundle loaded at Booster init: sponsor track docs, API docs, SDK examples, technical cheatsheet.

* Embeds all docs with text-embedding-004, stores in vector store keyed by booster\_id.
...
Model: gemini-1.5-pro. Max resource bundle: 200,000 tokens per Booster.

## **5.6 Metric Analyst Agent (Host)**

Analytics reporting agent that produces a structured, human-readable report on the current state of a Booster. Data-first: model only narrates and interprets data it is explicitly given.
...
Evaluates a Booster submission against host-defined judging criteria using the project's knowledge base. Produces structured scores and written remarks per category. Designed for consistency and explainability.
...
* Default criteria (customizable by host per Booster type):
...
Thin wrapper over code-reader sub-agent. Allows viewers and judges to ask questions about a project's codebase directly. Not re-implemented — calls queryCode(project\_id, question) from the code-reader module.
...
* ~~Filter by: category, tech stack, Booster participated in, date.~~
...
* Booster badges (which Boosters the project participated in with result).

* Team Details

## **6.3 Booster Page (Per Event)**

* Hero: banner, Booster name, type badge (Idea/Momentum/Capital), dates, prize pool.
...
* Booster overview cards (active, draft, completed).

* Per-Booster view: Booster Page editor, Builder Graph, Submissions, Judging Panel, Analytics.
...
* Booster Page → Projects Submitted → Project → Evaluate → Give Scores.
...
| :---- | :---- | :---- |
| User | id, email, username, oauth\_provider, role, created\_at | Has many Teams (owner), belongs to many Teams (member) |
| Team | id, name, owner\_id, created\_at | Has one owner, has many members, has many Projects |
| LoopsProfile | id, team\_id, name, tagline, description, logo\_url, website, github\_url, youtube\_url, tech\_stack\[\], category, colors{}, knowledge\_base\_id, momentum\_score, ai\_generated\_fields\[\] | Belongs to Team, has one KnowledgeBase, has many ProductVisuals |
| KnowledgeBase | id, project\_id, chunks\[\], embeddings\[\], sources{} | Belongs to LoopsProfile |
| Booster | id, host\_id, booster\_type (idea|momentum|capital), name, description, problem\_statement, start\_date, end\_date, judging\_criteria{}, status, leaderboard\_enabled | Belongs to Host, has many Submissions, has many Tracks |
| BoosterTrack | id, booster\_id, sponsor\_name, docs\_text, cheatsheet\_text, embeddings\[\] | Belongs to Booster (for Tech Buddy) |
| Submission | id, booster\_id, team\_id, project\_id, ai\_score{}, human\_score{}, momentum\_score, status | Belongs to Booster, Team, LoopsProfile |
| HostApplication | id, user\_id, booster\_type, event\_name, expected\_participants, contact, status, reviewed\_by | Belongs to User |
| JudgeInvite | id, booster\_id, judge\_user\_id, assigned\_tracks\[\], invited\_by | Belongs to Booster and User |
...
* Support 10,000 concurrent users during peak Booster hours.

* Knowledge base storage: horizontal sharding by project\_id via pgvector.

* AI agent jobs queued via BullMQ to prevent overload during mass submissions.

## **8.3 Security**

* All API endpoints protected by JWT (except public Loops Store and Booster pages).
...
* Tech Buddy: 50 queries/hour per authenticated builder per Booster.
...
* Capital Booster: who receives and manages actual grant disbursement — Germina Labs or the host?
