 ...
 *
 * Also seeds: 6 test users, 2 teams, 6 projects, knowledge bases, and CSV data.
 *
 * Uses the Supabase Admin API (service role key) to create auth users
 * and bypass RLS for all inserts.
 *
 * Usage:
 *   node supabase/scripts/seed-db.mjs
 *   npm run db:seed
 */
...
async function seedHackathons() {
  ...
  const hackathons = [
    ...
    // start_date in past, submission_deadline in future
    {
      ...
      technical_resources: [
        { url: "https://docs.soliditylang.org", description: "Solidity Documentation" },
        { url: "https://hardhat.org/docs", description: "Hardhat Testing Framework" },
        { url: "https://eips.ethereum.org/EIPS/eip-20", description: "ERC-20/721/1155 Patterns" },
      ],
      bounty_pool_summary: "$50K in prizes across 3 challenge tracks",
    ...
    // start_date and submission_deadline in past, results_date in future
    {
      ...
      technical_resources: [
        { url: "https://loops.house/docs", description: "Loops Platform Docs" },
        { url: "https://nextjs.org/docs", description: "Next.js Starter Kit" },
      ],
      judging_criteria: [
        {
...
// ---------------------------------------------------------------------------
// Step 5: Create projects (loops_profiles)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Step 10: Create hackathon resources (generated resource plans)
// ---------------------------------------------------------------------------
async function seedHackathonResources() {
  console.log("\n--- Seeding hackathon resources ---");
  ...
  const resources = [
        ...
        sections: [
          {
            ...
            items: [
              "Set up your development environment with Hardhat or Foundry",
              "Clone the starter template from the provided GitHub repo",
              "Join the #builders channel on Discord for real-time support",
            ],
          },
          {
            title: "API & SDK Access",
            items: [
              "Alchemy API key provided for all participants",
              "The Graph subgraph templates for indexing on-chain data",
              "OpenZeppelin Contracts library for secure smart contract patterns",
            ],
          },
        ],
        quickLinks: [
    ...
    },
    {
      ...
      content: {
        sections: [
          {
            ...
            items: [
              "Working demo or deployed prototype",
        ...
        quickLinks: [
          { label: "Loops Platform Docs", url: "https://loops.house/docs" },
          { label: "Next.js Starter", url: "https://nextjs.org/docs" },
        ],
      },
      source_urls: ["https://loops.house/docs", "https://nextjs.org/docs"],
      generated_at: new Date().toISOString(),
    },
  ];

  check("Hackathon resources", await supabase.from("hackathon_resources").insert(resources));
}

// ---------------------------------------------------------------------------
// Step 11: Create human evaluations (4 judges × 3 submissions for completed hackathon)
// ---------------------------------------------------------------------------
...
async function seedCSVData() {
  ...
  const csvPath = resolve(ROOT, "supabase", "scripts", "TestProjects.csv");
  let csvText;
  try {
...
async function main() {
  ...
  console.log("     - Technical resources + resource plan seeded");
  ...
  console.log("     - 1 judge (Frank), 2 speakers, resource plan seeded");
  ...
  console.log("  Hackathon Resources:2 (building + judging)");
