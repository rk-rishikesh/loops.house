...
import {
  ...
  Loader2, Check, ChevronRight, Pencil, Trash2, X, Save,
} from "lucide-react";
...
interface ProgramDraft {
  ...
  goals: string[];
  challenge_statements: { title: string; summary: string; difficulty?: string }[];
...
function FieldLabel({ children }: { children: React.ReactNode }) {
  return (
    <p className="text-[9px] tracking-[0.18em] uppercase font-bold text-[#2d4a3e]/40 mb-2" style={{ fontFamily: "'Inter', sans-serif" }}>
      {children}
    </p>
  );
}
...
function FieldHint({ children }: { children: React.ReactNode }) {
  return (
    <p className="text-xs text-[#2d4a3e]/45 mt-1.5 leading-relaxed" style={{ fontFamily: "Georgia, serif" }}>
      {children}
    </p>
  );
}
...
const inputBase: React.CSSProperties = {
  width: "100%", backgroundColor: "#d6cfc0", border: "none",
  borderRadius: 16, padding: "14px 18px",
  fontFamily: "Georgia, serif", fontSize: 15, color: "#2d4a3e", outline: "none",
};

const textareaBase: React.CSSProperties = { ...inputBase, resize: "none", lineHeight: 1.7 };
...
function NextButton({ onClick, label = "Next", disabled = false }: { onClick: () => void; label?: string; disabled?: boolean }) {
  return (
    ...
    >
      ...
      <span className="w-10 h-10 flex items-center justify-center rounded-full m-1.5" style={{ backgroundColor: "#d6cfc0" }}>
...
export default function HostBoostersPage() {
  ...
  const buildPayload = () => {
    const id = form.id || editingId || `booster_${Date.now()}`;
    const problem_statements = form.problem_statements.split("\n").map((s) => s.trim()).filter(Boolean);
    const technical_resources = form.technical_resources.map((r) => ({ url: r.url ?? "", description: r.description ?? "" }));
  ...
  const runAgents = async () => {
    ...
    try {
      ...
      if (pj.draft?.booster_id_suggestion && !form.id) set("id", pj.draft.booster_id_suggestion);
      setView("review");
    } catch (err) {
    ...
    <div className="min-h-screen" style={{ backgroundColor: "#f0ebe0" }}>
      ...
      {view === "form" && (
        <div className="px-10 pt-10 pb-24 max-w-[860px]">
          ...
          {/* Error banner */}
          {aiError && (
            <div className="mb-8 flex items-start gap-3 rounded-2xl px-5 py-4" style={{ backgroundColor: "rgba(200,60,60,0.07)", border: "1px solid rgba(200,60,60,0.15)" }}>
              <X size={13} className="shrink-0 mt-0.5" style={{ color: "#cc2222" }} />
              <p className="text-sm text-red-700" style={{ fontFamily: "Georgia, serif" }}>{aiError}</p>
            </div>
          )}
              ...
              <div id="step-resources">
                ...
                <div className="flex flex-col gap-3">
                  {form.technical_resources.map((res, i) => (
                    <div key={i} className="grid gap-2" style={{ gridTemplateColumns: "1fr 1.4fr 32px" }}>
                      <Input
                        value={res.url}
                        onChange={(v) => setForm((f) => ({ ...f, technical_resources: f.technical_resources.map((r, j) => j === i ? { ...r, url: v } : r) }))}
                        placeholder="https://docs.example.com"
                        type="url"
                      />
                      <Input
                        value={res.description}
                        onChange={(v) => setForm((f) => ({ ...f, technical_resources: f.technical_resources.map((r, j) => j === i ? { ...r, description: v } : r) }))}
                        placeholder="Short description"
                      />
                      <button
                        type="button"
                        onClick={() => setForm((f) => ({ ...f, technical_resources: f.technical_resources.filter((_, j) => j !== i) }))}
                  ...
                  <button
                    type="button"
                    onClick={() => setForm((f) => ({ ...f, technical_resources: [...f.technical_resources, { url: "", description: "" }] }))}
                    ...
                    style={{ fontFamily: "'Inter', sans-serif", color: "#2d4a3e" }}
                  >
            ...
            {formStep === "notes" && (
              <div id="step-notes">
                ...
                <div className="mt-8 flex items-center justify-between p-6 rounded-3xl" style={{ backgroundColor: "#2d4a3e" }}>
                  <div>
                    <p className="font-black text-[#f0ebe0] uppercase" style={{ fontFamily: "'Inter', sans-serif", fontSize: 15, letterSpacing: "-0.01em" }}>
                      Ready to generate
                    </p>
                    <p className="text-[#f0ebe0]/50 text-sm mt-1" style={{ fontFamily: "Georgia, serif" }}>
                  ...
                  >
                    <span className="pl-6 pr-3 py-4 text-[10px] tracking-[0.18em] uppercase font-bold text-[#2d4a3e] flex items-center gap-2" style={{ fontFamily: "'Inter', sans-serif" }}>
                      <Sparkles size={12} /> Generate Draft
                    </span>
                    <span className="w-10 h-10 flex items-center justify-center rounded-full m-1.5" style={{ backgroundColor: "#2d4a3e" }}>
      ...
      {/* ══════════════════════════════════════════════════════════════ */}
      {view === "generating" && (
        <div className="px-10 pt-10 pb-24 flex flex-col items-center justify-center" style={{ minHeight: "60vh" }}>
          <div
          ...
          <h2
            ...
            style={{ fontFamily: "'Inter', sans-serif", fontSize: "clamp(28px, 5vw, 56px)", letterSpacing: "-0.025em", lineHeight: 0.9 }}
          >
            DRAFTING
            <br />
            PROGRAM…
        ...
        <div className="px-10 pt-10 pb-24">
          ...
          <div className="grid gap-8 items-start" style={{ gridTemplateColumns: "1fr 320px" }}>
            ...
            <div className="flex flex-col gap-5">
              ...
              {programDraft?.draft.overview && (
                <div className="rounded-3xl p-7" style={{ backgroundColor: "#f5f2ea" }}>
                  <p className="text-[9px] tracking-[0.2em] uppercase font-bold text-[#2d4a3e]/40 mb-3" style={{ fontFamily: "'Inter', sans-serif" }}>Program Overview</p>
                  <p className="text-[#2d4a3e]/70 leading-relaxed" style={{ fontFamily: "Georgia, serif", fontSize: 15 }}>
                    {programDraft.draft.overview}
                  </p>
                </div>
              )}
              ...
              {(programDraft?.draft.goals?.length ?? 0) > 0 && (
                <div className="rounded-3xl p-7" style={{ backgroundColor: "#f5f2ea" }}>
                  <p className="text-[9px] tracking-[0.2em] uppercase font-bold text-[#2d4a3e]/40 mb-1" style={{ fontFamily: "'Inter', sans-serif" }}>Goals</p>
                  <h3 className="font-black text-[#2d4a3e] uppercase mb-5" style={{ fontFamily: "'Inter', sans-serif", fontSize: "clamp(16px, 2vw, 22px)", letterSpacing: "-0.02em" }}>
                    {programDraft!.draft.goals.length} Objectives.
                  </h3>
                  <div className="border-t border-[#2d4a3e]/12">
                    {programDraft!.draft.goals.map((g, i) => (
                      <div key={i} className="flex items-start gap-4 py-4 border-b border-[#2d4a3e]/08">
                        <span className="font-black text-[#2d4a3e]/18 shrink-0" style={{ fontFamily: "'Inter', sans-serif", fontSize: 12, width: 24 }}>{String(i + 1).padStart(2, "0")}</span>
                        <p className="text-[#2d4a3e]/70 text-sm leading-relaxed" style={{ fontFamily: "Georgia, serif" }}>{g}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {(programDraft?.draft.challenge_statements?.length ?? 0) > 0 && (
                <div className="rounded-3xl p-7" style={{ backgroundColor: "#f5f2ea" }}>
                  <p className="text-[9px] tracking-[0.2em] uppercase font-bold text-[#2d4a3e]/40 mb-1" style={{ fontFamily: "'Inter', sans-serif" }}>Challenge Statements</p>
                  <h3 className="font-black text-[#2d4a3e] uppercase mb-5" style={{ fontFamily: "'Inter', sans-serif", fontSize: "clamp(16px, 2vw, 22px)", letterSpacing: "-0.02em" }}>
                  ...
                  <div className="border-t border-[#2d4a3e]/12">
                    {programDraft!.draft.challenge_statements.map((c, i) => (
                      <div key={i} className="flex items-start gap-4 py-5 border-b border-[#2d4a3e]/08">
                        <span className="font-black text-[#2d4a3e]/18 shrink-0 pt-0.5" style={{ fontFamily: "'Inter', sans-serif", fontSize: 12, width: 24 }}>{String(i + 1).padStart(2, "0")}</span>
                        <div>
                          <p className="font-semibold text-[#2d4a3e] text-sm mb-1" style={{ fontFamily: "'Inter', sans-serif" }}>{c.title}</p>
                          <p className="text-[#2d4a3e]/60 text-sm leading-relaxed" style={{ fontFamily: "Georgia, serif" }}>{c.summary}</p>
                          {c.difficulty && (
                            <span className="inline-block mt-2 text-[8px] tracking-[0.12em] uppercase font-bold px-2.5 py-1 rounded-sm" style={{ backgroundColor: "rgba(45,74,62,0.08)", color: "#2d4a3e", fontFamily: "'Inter', sans-serif" }}>
                              {c.difficulty}
                            </span>
                          )}
                        </div>
              ...
              {resourcePlan?.resources.technical_cheatsheet && (
                <div className="rounded-3xl p-7" style={{ backgroundColor: "#2d4a3e" }}>
                  <p className="text-[9px] tracking-[0.2em] uppercase font-bold text-[#f0ebe0]/38 mb-3" style={{ fontFamily: "'Inter', sans-serif" }}>Technical Cheatsheet</p>
                  <div className="rounded-2xl p-5 overflow-auto max-h-64" style={{ backgroundColor: "rgba(240,235,224,0.06)", border: "1px solid rgba(240,235,224,0.07)" }}>
              ...
              {(resourcePlan?.resources.tracks?.length ?? 0) > 0 && (
                <div className="rounded-3xl p-7" style={{ backgroundColor: "#f5f2ea" }}>
                  <p className="text-[9px] tracking-[0.2em] uppercase font-bold text-[#2d4a3e]/40 mb-1" style={{ fontFamily: "'Inter', sans-serif" }}>Resource Tracks</p>
                  <h3 className="font-black text-[#2d4a3e] uppercase mb-5" style={{ fontFamily: "'Inter', sans-serif", fontSize: "clamp(16px, 2vw, 22px)", letterSpacing: "-0.02em" }}>
                  ...
                  <div className="border-t border-[#2d4a3e]/12">
                    {resourcePlan!.resources.tracks.map((t, i) => (
                      <div key={i} className="flex items-start gap-4 py-5 border-b border-[#2d4a3e]/08">
                        <span className="font-black text-[#2d4a3e]/18 shrink-0 pt-0.5" style={{ fontFamily: "'Inter', sans-serif", fontSize: 12, width: 24 }}>{String(i + 1).padStart(2, "0")}</span>
                        <div>
                          <p className="font-semibold text-[#2d4a3e] text-sm mb-1" style={{ fontFamily: "'Inter', sans-serif" }}>{t.name}</p>
                          <p className="text-[#2d4a3e]/60 text-sm leading-relaxed" style={{ fontFamily: "Georgia, serif" }}>{t.description}</p>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <aside className="sticky top-[81px] flex flex-col gap-4">
              ...
              {/* Save CTA */}
              <div className="rounded-3xl p-7" style={{ backgroundColor: "#2d4a3e" }}>
                <p className="font-black text-[#f0ebe0] uppercase mb-2" style={{ fontFamily: "'Inter', sans-serif", fontSize: 16, letterSpacing: "-0.01em" }}>
                  Looks good?
                </p>
                ...
                <button
                  type="button"
                  onClick={() => { setFormStep("notes"); setView("form"); }}
                  ...
                  style={{ backgroundColor: "transparent", color: "rgba(240,235,224,0.4)", padding: "10px 24px", fontFamily: "'Inter', sans-serif", fontSize: 10, letterSpacing: "0.18em", fontWeight: 700, textTransform: "uppercase" }}
                >
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    ...
                    <div key={label} className="rounded-xl p-3" style={{ backgroundColor: "rgba(45,74,62,0.08)" }}>
                      ...
                      <p className="text-[9px] tracking-[0.12em] uppercase font-bold text-[#2d4a3e]/40 mt-1.5" style={{ fontFamily: "'Inter', sans-serif" }}>{label}</p>
                    </div>
                  ))}
                </div>
              </div>
