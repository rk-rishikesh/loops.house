...
export default function ProjectHubPage() {
  ...
  const [activeBoosterType, setActiveBoosterType] = useState<BoosterType>("idea");

  const { data: boosters = [], isLoading: boostersLoading } = useBoosters(activeBoosterType);
    ...
    <div className="relative h-full bg-[#f0ebe0] font-sans">
      ...
      <div className="px-10 pt-8">
        ...
        >
          ...
          <button
            type="button"
            onClick={() => router.push("/builder/boosters/idea")}
            className="flex-1 min-w-0 py-4 flex items-center justify-center border-r border-[#1a1a1a] bg-transparent hover:bg-[#e1dbcf] cursor-pointer"
          >
            <span>Boosters</span>
          </button>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-6">
                          ...
                          <div>
                            <p className="text-[10px] tracking-[0.18em] uppercase font-semibold leading-none mb-1"
                              style={{ fontFamily: "'Inter', sans-serif", color: active ? "#2d4a3e" : "rgba(45,74,62,0.6)" }}>
                              {type.toUpperCase()}
                            </p>
                            <p className="text-[10px] tracking-[0.18em] uppercase font-semibold leading-none"
                              style={{ fontFamily: "'Inter', sans-serif", color: active ? "#2d4a3e" : "rgba(45,74,62,0.4)" }}>
                              BOOSTER
                            </p>
                          </div>
                        </div>
              ...
              <div className="col-span-3 col-start-10 self-start mt-24">
                ...
                <nav className="mb-10">
                  {[
                    ...
                    { label: "Boosters", href: "/builder/boosters/idea" },
                ...
                {(["idea", "momentum", "capital"] as BoosterType[]).map((type) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      onClick={() => setActiveBoosterType(type)}
                      className={`px-3 py-1.5 text-[10px] tracking-[0.18em] uppercase font-bold rounded-full border cursor-pointer transition-colors ${
                        active
                          ? "bg-[#2d4a3e] text-[#f0ebe0] border-[#2d4a3e]"
                          : "bg-transparent text-[#2d4a3e] border-[#2d4a3e]/30 hover:border-[#2d4a3e]"
                      }`}
                      style={{ fontFamily: "'Inter', sans-serif" }}
                    >
                      {type}
            ...
            {boostersLoading ? (
              <div className="mt-4 space-y-3">
                {[1, 2].map((i) => (
                  <div
                    key={i}
                    className="rounded-2xl px-6 py-5 bg-[#d6cfc0] animate-pulse"
                  />
                ))}
              </div>
            ) : boosters.length === 0 ? (
              <div
                ...
                style={{ backgroundColor: "#e8e2d4", fontFamily: "Georgia, serif" }}
              >
                No boosters of this type yet. Hosts can create new programs from the Host dashboard.
              </div>
            ) : (
              <ul className="mt-4 space-y-3 max-w-3xl">
                {boosters.map((b) => (
                  <li key={b.id}>
                    <Link
                      href={`/boosters/${activeBoosterType}/${b.id}`}
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3 shrink-0">
                          <button
                            ...
                            onClick={(e) => {
                              e.preventDefault();
                              router.push(`/builder/boosters/${activeBoosterType}/${b.id}/submit`);
                            }}
