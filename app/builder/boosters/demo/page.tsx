...
import {
  ArrowLeft, ArrowUpRight, Calendar, Trophy,
  Clock, Zap, Search,
} from "lucide-react";
import { useBoosters } from "@/lib/queries";
import type { BoosterType, StoredBooster } from "@/lib/storage";
  ...
  return (
    <div className="flex items-baseline justify-between mb-6">
      <h2 className="font-black text-[#2d4a3e] uppercase"
        style={{ fontFamily: "'Inter', sans-serif", fontSize: "clamp(22px, 3vw, 36px)", letterSpacing: "-0.02em" }}>
        {label}
      </h2>
      {count !== undefined && (
        <span className="text-[10px] tracking-widest uppercase font-bold text-[#2d4a3e]/30"
          style={{ fontFamily: "'Inter', sans-serif" }}>{count}</span>
      )}
    </div>
  );
}
...
function Skeleton({ tall = false }: { tall?: boolean }) {
  return <div className="rounded-2xl animate-pulse" style={{ backgroundColor: "#d6cfc0", height: tall ? 320 : 200 }} />;
}

/* ─── Featured hero card ─────────────────────────────────────────── */
function FeaturedCard({ b }: { b: StoredBooster & { _type: BoosterType } }) {
  return (
    <Link href={`/boosters/${b._type}/${b.id}`} className="no-underline group block">
      <div className="rounded-3xl p-8 flex flex-col justify-between transition-all duration-300 group-hover:scale-[1.01] relative overflow-hidden"
        style={{ backgroundColor: "#2d4a3e", minHeight: 340 }}>
        <span className="absolute right-4 bottom-0 select-none pointer-events-none font-black text-[#f0ebe0]/04"
          style={{ fontFamily: "'Inter', sans-serif", fontSize: "clamp(120px, 18vw, 220px)", letterSpacing: "-0.05em", lineHeight: 0.85 }}>★</span>
        <div className="flex items-start justify-between gap-4">
          <StatusPill status="featured" />
          <span className="w-12 h-12 flex items-center justify-center rounded-full shrink-0"
            style={{ backgroundColor: "#d6cfc0" }}>
            <ArrowUpRight size={18} style={{ color: "#2d4a3e" }} />
          </span>
        </div>
        <div className="mt-10 relative z-10">
          <p className="text-[9px] tracking-[0.2em] uppercase font-bold text-[#f0ebe0]/38 mb-3"
            style={{ fontFamily: "'Inter', sans-serif" }}>{TYPE_LABELS[b._type].nav}</p>
          <h2 className="font-black text-[#f0ebe0] uppercase leading-[0.88] mb-4"
            style={{ fontFamily: "'Inter', 'Helvetica Neue', sans-serif", fontSize: "clamp(28px, 4vw, 52px)", letterSpacing: "-0.025em" }}>
            {b.name}
          </h2>
          {b.theme && (
            <p className="text-[#f0ebe0]/55 text-sm leading-relaxed mb-6 max-w-[480px]"
              style={{ fontFamily: "Georgia, serif" }}>{b.theme}</p>
          )}
          <div className="flex items-center gap-5 flex-wrap">
            ...
            {b.timeline && (
              <div className="flex items-center gap-2">
                <Calendar size={11} style={{ color: "#d6cfc0", opacity: 0.6 }} />
                <span className="text-[11px] text-[#f0ebe0]/50" style={{ fontFamily: "Georgia, serif" }}>
...
function OngoingRow({ b }: { b: StoredBooster & { _type: BoosterType } }) {
  return (
    <Link href={`/boosters/${b._type}/${b.id}`} className="no-underline group block">
      <div className="rounded-2xl px-8 py-6 flex items-center gap-6 transition-all duration-200 group-hover:scale-[1.01]"
        style={{ backgroundColor: "#d6cfc0" }}>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-3 mb-1.5">
            ...
            <span className="text-[9px] tracking-[0.12em] uppercase font-bold text-[#2d4a3e]/40"
              style={{ fontFamily: "'Inter', sans-serif" }}>{TYPE_LABELS[b._type].nav}</span>
          </div>
...
function GridCard({ b, status }: { b: StoredBooster & { _type: BoosterType }; status: EventStatus }) {
  const isPast = status === "past";
  return (
    <Link href={`/boosters/${b._type}/${b.id}`} className="no-underline group block">
      <div className="rounded-2xl p-6 flex flex-col justify-between transition-all duration-200 group-hover:scale-[1.015]"
        style={{ backgroundColor: isPast ? "#f5f2ea" : "#d6cfc0", minHeight: 200 }}>
        <div className="flex items-start justify-between gap-3 mb-4">
          <StatusPill status={status} />
          <span className="w-9 h-9 flex items-center justify-center rounded-full shrink-0 transition-all group-hover:scale-105"
            style={{ backgroundColor: isPast ? "rgba(45,74,62,0.1)" : "#2d4a3e" }}>
            <ArrowUpRight size={13} style={{ color: isPast ? "rgba(45,74,62,0.4)" : "#f0ebe0" }} />
          </span>
        </div>
        <div>
          <p className="text-[9px] tracking-[0.15em] uppercase font-bold mb-2"
          ...
          <h3 className="font-black uppercase leading-tight mb-2"
            style={{ fontFamily: "'Inter', sans-serif", fontSize: "clamp(14px, 1.8vw, 18px)", letterSpacing: "-0.02em", color: isPast ? "rgba(45,74,62,0.5)" : "#2d4a3e" }}>
            {b.name}
          </h3>
          {b.theme && (
            <p className="text-xs leading-relaxed mb-3"
              style={{ fontFamily: "Georgia, serif", color: isPast ? "rgba(45,74,62,0.35)" : "rgba(45,74,62,0.6)" }}>
              {b.theme.slice(0, 80)}{b.theme.length > 80 ? "…" : ""}
            </p>
          )}
...
export default function BoosterTypePage() {
  ...
  const { data: ideaList     = [], isLoading: l1 } = useBoosters("idea");
  const { data: momentumList = [], isLoading: l2 } = useBoosters("momentum");
  const { data: capitalList  = [], isLoading: l3 } = useBoosters("capital");
  const loading = l1 || l2 || l3;

  const allBoosters = useMemo(() => [
    ...ideaList.map((b)     => ({ ...b, _type: "idea"     as BoosterType })),
    ...momentumList.map((b) => ({ ...b, _type: "momentum" as BoosterType })),
    ...capitalList.map((b)  => ({ ...b, _type: "capital"  as BoosterType })),
  ], [ideaList, momentumList, capitalList]);
  ...
  const filtered = useMemo(() => {
    ...
    if (activeFilter !== "all") list = list.filter((b) => b._type === activeFilter);
    if (search.trim()) {
  ...
  const withStatus = useMemo(() =>
    filtered.map((b, idx) => ({ ...b, _status: deriveStatus(b, idx) as EventStatus })),
    [filtered]
  );
  ...
  const isEmpty  = !loading && withStatus.length === 0;

  return (
    <main className="min-h-screen" style={{ backgroundColor: "#f0ebe0" }}>

      {/* ── Nav ─────────────────────────────────────────────────────── */}
      <div className="sticky top-0 z-50 px-10 py-5 flex items-center justify-between gap-6"
        ...
        {/* Type filter pills */}
        <div className="flex items-center gap-1 rounded-full p-1" style={{ backgroundColor: "rgba(45,74,62,0.07)" }}>
          {(["all", ...TYPES] as const).map((t) => {
            const isActive = activeFilter === t;
            const label    = t === "all" ? "All" : TYPE_LABELS[t as BoosterType].nav;
            return (
              <button key={t} type="button" onClick={() => setActiveFilter(t)}
                ...
                style={{ fontFamily: "'Inter', sans-serif", padding: "8px 14px", backgroundColor: isActive ? "#2d4a3e" : "transparent", color: isActive ? "#f0ebe0" : "rgba(45,74,62,0.5)" }}>
                {label}
              </button>
            );
          })}
        ...
        {/* Search */}
        <div className="flex items-center gap-2 rounded-full px-4 py-2.5 flex-1 max-w-xs"
      ...
      <div className="px-10 pt-10 pb-24">
        ...
        <div className="mb-14 flex items-start justify-between gap-8">
          ...
          {!loading && (
            <div className="flex gap-3 shrink-0 mt-2">
              {[
                { label: "Open Now", value: ongoing.length,    bg: "#2d4a3e", fg: "#f0ebe0", fgMid: "rgba(240,235,224,0.5)" },
                { label: "Upcoming", value: upcoming.length,   bg: "#d6cfc0", fg: "#2d4a3e", fgMid: "rgba(45,74,62,0.45)"   },
                { label: "Total",    value: withStatus.length, bg: "#f5f2ea", fg: "#2d4a3e", fgMid: "rgba(45,74,62,0.45)"   },
              ].map(({ label, value, bg, fg, fgMid }) => (
                <div key={label} className="rounded-2xl px-5 py-5 text-center" style={{ backgroundColor: bg, minWidth: 90 }}>
                  <p className="font-black leading-none mb-1"
                    style={{ fontFamily: "'Inter', sans-serif", fontSize: 28, letterSpacing: "-0.03em", color: fg }}>
                    {value}
                  </p>
                  <p className="text-[8px] tracking-[0.14em] uppercase font-bold"
                    style={{ fontFamily: "'Inter', sans-serif", color: fgMid }}>{label}</p>
                </div>
              ))}
            </div>
          )}
        ...
        <p className="text-[#2d4a3e]/55 mb-14 max-w-[480px] leading-relaxed"
          style={{ fontFamily: "Georgia, serif", fontSize: "clamp(14px, 1.4vw, 17px)" }}>
          Hackathons, grants, and build programs — browse by status or search by topic.
        </p>
        ...
        {isEmpty && (
          <div className="py-24 text-center rounded-3xl" style={{ backgroundColor: "#f5f2ea" }}>
            ...
            <p className="text-[#2d4a3e]/50 text-sm" style={{ fontFamily: "Georgia, serif" }}>
              {search ? "Try a different search term." : "Check back soon — hosts are adding programs."}
            </p>
            {search && (
            ...
            {ongoing.length > 0 && (
              <section>
                ...
                <div className="flex flex-col gap-4">
                  {ongoing.map((b) => <OngoingRow key={b.id} b={b} />)}
                </div>
              </section>
            )}
            ...
            {upcoming.length > 0 && (
              <section>
                ...
                <div className="grid grid-cols-3 gap-4">
                  {upcoming.map((b) => <GridCard key={b.id} b={b} status="upcoming" />)}
                </div>
              </section>
            )}
              ...
              <section>
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <p className="text-[10px] tracking-[0.2em] uppercase font-bold text-[#2d4a3e]/30"
                    style={{ fontFamily: "'Inter', sans-serif" }}>Past Programs</p>
                  <div className="flex-1 h-px" style={{ backgroundColor: "rgba(45,74,62,0.12)" }} />
                </div>
                <div className="grid grid-cols-4 gap-3">
                  {past.map((b) => <GridCard key={b.id} b={b} status="past" />)}
                </div>
              </section>
            )}
          </div>
