...
export type HackathonPhase = "upcoming" | "building" | "judging" | "completed" | "finalized";
...
export function computePhase(hackathon: {
  ...
  // If finalized_at is set AND we're past results_date → finalized
  if (hackathon.finalized_at && results && now >= results) {
    return "finalized";
  }
