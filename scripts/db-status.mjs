...
if (issues.length === 0) {
  ...
  if (!allTablesOk || !allFunctionsOk) {
    ...
    console.log('  Option B: Copy SQL from supabase/migrations/ into the Supabase Dashboard SQL Editor');
    if (!allFunctionsOk && allTablesOk) {
      console.log('  (Tables exist but functions are missing — run supabase/migrations/003_functions.sql)');
    }
  }
  if (!allBucketsOk) {
    console.log('  To create storage buckets: node scripts/setup-storage.mjs');
  }
}
