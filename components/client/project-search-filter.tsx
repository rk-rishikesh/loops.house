...
function ProjectRow({ project: p, index }: { project: StoredProject; index: number }) {
  return (
    <Link href={`/viewer/projects/${p.project_id}`} className="no-underline group">
      ...
      >
        <p className="font-bold text-[#2d4a3e] pt-0.5" style={{ fontFamily: "'Inter', sans-serif", fontSize: "clamp(13px, 1.4vw, 15px)" }}>
          {String(index + 1).padStart(2, "0")}.
        </p>
        <div>
          <p className="font-semibold text-[#2d4a3e] mb-1.5 leading-snug" style={{ fontFamily: "'Inter', sans-serif", fontSize: "clamp(13px, 1.3vw, 15px)" }}>
            {p.name}
            {p.tagline && (
              <>
                <br />
                <span className="font-normal text-[#2d4a3e]/50" style={{ fontFamily: "Georgia, serif", fontSize: "0.95em" }}>{p.tagline}</span>
              </>
            )}
          </p>
          {(p.category || (p.tech_stack_tags ?? []).length > 0) && (
            <div className="flex flex-wrap gap-1.5 mt-2">
              {p.category && (
                <span className="text-[8px] tracking-[0.12em] uppercase font-bold px-2.5 py-1 rounded-sm" style={{ backgroundColor: "rgba(45,74,62,0.1)", color: "#2d4a3e", fontFamily: "'Inter', sans-serif" }}>{p.category}</span>
              )}
              {(p.tech_stack_tags ?? []).slice(0, 3).map((t) => (
                <span key={t} className="text-[10px] px-2 py-0.5 rounded-sm" style={{ backgroundColor: "rgba(45,74,62,0.06)", color: "rgba(45,74,62,0.55)", fontFamily: "Georgia, serif" }}>{t}</span>
              ))}
            </div>
          )}
        </div>
        <div className="flex items-start gap-4">
          <p className="text-[#2d4a3e]/50 leading-relaxed text-sm" style={{ fontFamily: "Georgia, serif" }}>
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-4 mb-6">
        ...
        {hasFilters && (
          <div className="flex items-center gap-3">
            <span className="text-[11px] text-[#2d4a3e]/45" style={{ fontFamily: "Georgia, serif" }}>
              {filtered.length} result{filtered.length !== 1 ? "s" : ""}
            </span>
            <button
              type="button"
              onClick={() => { setSearch(""); setCategoryFilter(""); }}
              className="inline-flex items-center gap-1.5 text-[10px] tracking-widest uppercase font-bold text-[#2d4a3e]/40 hover:text-[#2d4a3e] transition-colors bg-transparent border-none cursor-pointer"
              style={{ fontFamily: "'Inter', sans-serif" }}
            >
              <X size={10} /> Clear
            </button>
      ...
      <div className="relative w-full max-w-[300px]">
        <Search size={14} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#2d4a3e]/35 pointer-events-none" />
        <input
          type="text"
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          placeholder="Search projects\u2026"
          className="w-full pl-9 pr-8 py-2.5 rounded-sm bg-transparent text-[13px] text-[#2d4a3e] placeholder-[#2d4a3e]/35 outline-none transition-all"
          style={{ fontFamily: "'Inter', sans-serif", border: "1.5px solid rgba(45,74,62,0.2)", letterSpacing: "0.01em" }}
        />
        {search && (
          <button type="button" onClick={() => setSearch("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#2d4a3e]/40 hover:text-[#2d4a3e] transition-colors">
            <X size={12} />
          </button>
        )}
      </div>
      ...
      {/* ── Table header ────────────────────────────────────────────── */}
      <div className="grid border-b border-t border-[#2d4a3e]/20 py-3 mb-0 mt-6" style={{ gridTemplateColumns: "80px 1fr 1.6fr 56px", gap: "0 24px" }}>
      ...
      {filtered.length === 0 ? (
        <div className="py-20 text-center border-b border-[#2d4a3e]/12">
          <div className="inline-flex items-center justify-center w-12 h-12 rounded-xl mb-5" style={{ backgroundColor: "rgba(45,74,62,0.08)", color: "#2d4a3e" }}>
            <Code2 size={20} />
          </div>
          <p className="font-bold text-[#2d4a3e] mb-2" style={{ fontFamily: "'Inter', sans-serif", fontSize: 15 }}>
            {hasFilters ? "No matches found." : "No projects yet."}
          </p>
          <p className="text-sm text-[#2d4a3e]/50 mb-8 leading-relaxed" style={{ fontFamily: "Georgia, serif" }}>
