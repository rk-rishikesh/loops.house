    ...
    <section className="w-full">
      <div className="grid gap-8 lg:grid-cols-[1.4fr,1fr]">
        ...
        <div className="flex flex-col gap-8 py-4">
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <span className="px-2 py-0.5 rounded bg-[#0F2C23] text-[#E2FEA5] text-[9px] font-black tracking-widest uppercase" style={{ fontFamily: PX }}>
                Active Session
              </span>
            ...
            >
              Building Project<br />Intelligence.
            </h2>
            <p
            ...
            >
              ...
              dependencies, analyzing user flows, and indexing visual context into a 
              high-fidelity knowledge graph.
            </p>
          </div>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      <h3 className="font-bold text-[15px] text-[#0F2C23]" style={{ fontFamily: PX }}>
                        {friendlyLabel[entry.step]}
                      </h3>
                      <span className="text-[9px] font-black uppercase tracking-widest text-[#0F2C23]/40" style={{ fontFamily: PX }}>
                        {status === "done" ? "Verified" : status === "started" ? "Processing" : status === "failed" ? "Interrupt" : "Pending"}
                      </span>
                    </div>
                    <p className="text-[12px] leading-relaxed text-[#0F2C23]/50" style={{ fontFamily: FN }}>
                      {isFailed ? entry.error : (
                        <>
                          {idx === 0 && "Cloning repository and executing deep AST traversal."}
                          {idx === 1 && "Analyzing demo frames to identify core product archetypes."}
                          {idx === 2 && "Extracting design tokens and high-level component hierarchy."}
        ...
        <div className="flex flex-col gap-6 py-4">
          <div className="flex-1 rounded-[40px] p-8 flex flex-col gap-8 relative overflow-hidden" 
               style={{ backgroundColor: "#0F2C23" }}>
            
            <div className="relative z-10 flex-1 flex flex-col">
              <div className="flex items-center justify-between mb-6">
                <p className="text-[10px] font-black uppercase tracking-[0.3em] text-[#E2FEA5]/40" style={{ fontFamily: PX }}>
                  Live Telemetry
                </p>
                ...
                <AnimatePresence mode="popLayout">
                  {statusByStep.map((entry) => {
                    ...
                    return (
                      ...
                      >
                        <span className="text-[#E2FEA5]/40">[{new Date().toLocaleTimeString([], { hour12: false, hour: '2-digit', minute: '2-digit', second: '2-digit' })}]</span>
                        <span className="text-[#E2FEA5] font-bold uppercase tracking-tighter" style={{ fontSize: 9 }}>{entry.step.replace('-', '_')}</span>
                        <span className="text-[#F8FFE8]/80">
                          {entry.status === "done" && "execution_complete. context_synced."}
                          {entry.status === "started" && "establishing_handshake. streaming_data..."}
            ...
            <p className="relative z-10 text-[9px] text-center font-black uppercase tracking-[0.25em] text-[#E2FEA5]/30" style={{ fontFamily: PX }}>
